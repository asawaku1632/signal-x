  ...
  try {
    const { rows } = await pool.query(`
      SELECT
        ...
        ROUND(AVG(volatility),2)::numeric AS average_volatility
    ...
    const ranking = rows
      .map((row) => {
        ...
        return {
          volatilityBand: row.volatility_band,
    ...
    return NextResponse.json({
      success: true,
      aiPowerVersion:
        "V13.10_VOLATILITY_ANALYTICS",
      bandCount: ranking.length,
      ranking,
    });
  } catch (error) {
    console.error(
      "volatility analytics error:",
      error
    );
