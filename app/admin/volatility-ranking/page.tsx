...
export default async function VolatilityRankingPage() {
  const data = await getVolatilityAnalytics();
  const ranking: VolatilityBand[] = data.ranking ?? [];

  return (
    <main style={{ padding: 24, maxWidth: 1100, margin: "0 auto" }}>
      <h1 style={{ fontSize: 28, fontWeight: 900 }}>
        🌊 Volatility Ranking AI
      </h1>

      <p style={{ color: "#666", marginTop: 8 }}>
        V13.10 / ボラティリティ別の勝率・AI BONUS・Confidenceをランキング表示
      </p>

      <section style={{ marginTop: 24, display: "grid", gap: 14 }}>
        {ranking.map((item) => (
          <div
            key={item.volatilityBand}
            style={{
            ...
            >
              <div>
                <div style={{ fontSize: 26, fontWeight: 900 }}>
                  {item.rankLabel} {getBandLabel(item.volatilityBand)}
                </div>
